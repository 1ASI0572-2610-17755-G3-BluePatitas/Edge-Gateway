...
base_latitude = -12.103638802091739
base_longitude = -76.96249460429247
offset_latitude = 0.0
...
def evaluar_y_notificar_geocerca(lat, lng, target_id=None):
    try:
        ...
        if res_zones.status_code == 200:
            ...
            for zone in zones:
                ...
                zone_target_id = zone.get("targetId")
                ...
                if (
                    ...
                    # Si recibimos target_id especifico, solo evaluar ese
                    if target_id is not None and zone_target_id != target_id:
                        continue

                    dist = distance_meters(
                        lat, lng, lat_center, lng_center
                    )
                    print(
                        f"[Geofence Check] Zone: {zone.get('name')} | Target: {zone_target_id} | Distance: {dist:.2f}m | Radius: {radius:.2f}m"
                    )
                    if dist > radius:
                        print(
                            f"[Geofence Breach] Target {zone_target_id} is OUTSIDE the geofence!"
                        )
                        alert_payload = {
                            "targetId": zone_target_id,
                            "latitude": lat,
                            "longitude": lng,
                        }
...
def recibir_ubicacion():
    ...
    try:
        ...
        evaluar_y_notificar_geocerca(lat_to_save, lng_to_save)
...
def configurar_simulador():
    ...
    try:
        datos = request.get_json() or {}
        base_latitude = datos.get("latitude", -12.103638802091739)
        base_longitude = datos.get("longitude", -76.96249460429247)
        offset_latitude = 0.0
        offset_longitude = 0.0
        ...
        # Evaluar inmediatamente la geocerca
        evaluar_y_notificar_geocerca(base_latitude, base_longitude, active_target_id)
...
def alejar_simulacion():
    ...
    evaluar_y_notificar_geocerca(base_latitude + offset_latitude, base_longitude + offset_longitude, active_target_id)
